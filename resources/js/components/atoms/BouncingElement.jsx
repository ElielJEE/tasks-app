...
const BouncingLogo = ({ txtOne, txtTwo }) => {
	const [position, setPosition] = useState({ x: 0, y: 0 });
	const [direction, setDirection] = useState({ dx: 2, dy: 2 });
	const [collisionText, setCollisionText] = useState()

	const messages = [
		"La vida es un instante, haz que valga la pena.",
		"Vive cada día como si fuera el último, pero aprende como si vivieras para siempre.",
		"Aprovecha el presente, el pasado ya no está y el futuro es incierto.",
		"No cuentes los días, haz que los días cuenten.",
		"Lo único seguro en la vida es este momento.",
		"No te preocupes tanto por el destino, disfruta del viaje.",
		"El tiempo es oro, gástalo sabiamente.",
		"Haz lo que amas y hazlo ahora.",
		"Cada segundo es una nueva oportunidad para comenzar de nuevo.",
		"No te eches un pedo con diarrea."
	];
		...
		const moveLogo = () => {
			setPosition((prev) => {
				...
				if (x + logoWidth >= containerWidth || x <= 0) {
					dx = -dx; // Cambio horizontal
					collisionDetected = true;
					setCollisionText(messages[Math.floor(Math.random() * messages.length)]);
				}
				if (y + logoHeight >= containerHeight || y <= 0) {
					dy = -dy; // Cambio vertical
					collisionDetected = true;
					setCollisionText(messages[Math.floor(Math.random() * messages.length)]);
				}
				...
				if (x + logoWidth >= containerWidth || x <= 0 || y + logoHeight >= containerHeight || y <= 0) {
					document.getElementById('nota').style.color = `hsl(${Math.random() * 360}, 50%, 50%)`;
					document.getElementById('advise').style.color = `hsl(${Math.random() * 360}, 50%, 50%)`;
					document.getElementById('logo').style.fill = `hsl(${Math.random() * 360}, 50%, 50%)`;
				}

				return {
					x: Math.max(0, Math.min(x + dx, containerWidth - logoWidth)),
					y: Math.max(0, Math.min(y + dy, containerHeight - logoHeight)),
				};
			});
		};

		const interval = setInterval(moveLogo, 16); // Aproximadamente 60 FPS
