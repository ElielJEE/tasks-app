...
export default function TaskCard({ title, description, difficulty, id, objectives }) {
  ...
  const handleChangeObjectivesUpdate = (e, index) => {
    e.preventDefault();
    const newObjectives = [...taskDataUpdate.objectives];
  ...
  const handleObjectiveKeyPress = (e, index) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (taskDataUpdate.objectives[index].description.trim() !== '') {
        addObjective(e);
      }
    }
  };
      ...
      <div className={`task-card ${isCompleted ? 'completed' : ''}`}>
        ...
        <div className="task-card__info-container">
          {
            active === 1 ? (
              <>
                <input type="text" name="title" value={taskDataUpdate.title} className='task-card__info-container__task-card-input'
                  onChange={handleChange}
                />
                <textarea name="description" value={taskDataUpdate.description} className='task-card__info-container__task-card-input'
                  onChange={handleChange}
                />
                <select
                  name="difficulty"
                  className='task-card__info-container__task-card-input'
                  value={taskDataUpdate.difficulty}
                  onChange={handleChange}
                >
                ...
                <ul className="task-card__info-container__task-objectives-list">
                  {
                    <>
                      {
                        taskDataUpdate.objectives.map((item, index) => (
        ...
        </div >
        {
          active === 1 ? (
            ...
            <>
              <button className="task-card__edit-btn" onClick={e => {
                e.preventDefault();
                activeHandle(1);
              }}>
